...
try{await window._fb.signInWithPopup(window._fb.auth,window._fb.googleProvider);_postAuthRedirect();}catch(e){alert('Google sign-in error: '+e.code+'\n'+e.message);}}
...
if(!password){showAuthError('errorBanner','Please enter your password.');return;}
if(!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)){showAuthError('errorBanner','Please enter a valid email address.');return;}
const btn=document.getElementById('loginBtn');btn.classList.add('loading');btn.disabled=true;try{await window._fb.signInWithEmailAndPassword(window._fb.auth,email,password);_postAuthRedirect();}catch(e){showAuthError('errorBanner',e.code==='auth/wrong-password'||e.code==='auth/user-not-found'?'Incorrect email or password.':e.message);}finally{btn.classList.remove('loading');btn.disabled=false;}}
...
function _postAuthRedirect(){var pending=sessionStorage.getItem('sp_pending_cart');if(pending){sessionStorage.removeItem('sp_pending_cart');try{var item=JSON.parse(pending);showPage('pricing');setTimeout(function(){if(window.spAddToCart)window.spAddToCart(item.type,item.id);},120);}catch(e){_postAuthRedirect();}}else{_postAuthRedirect();}}
